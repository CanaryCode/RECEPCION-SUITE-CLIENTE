...
function authMiddleware(req, res, next) {
    ...
    // Status y Login son públicos para permitir la UI inicial
...
async function fetchRemoteAgent(endpoint, method = 'GET', payload = null, adminPass = null) {
    let directError;
    // 1. Intento directo al agente (puerto 3001)
    try {
        if (method === 'GET') {
            return await fetchUrl(`http://www.desdetenerife.com:3001${endpoint}`, adminPass, 3000);
        } else {
            return await fetchUrlPost(`http://www.desdetenerife.com:3001${endpoint}`, payload, adminPass);
        }
    } catch (e) {
        directError = e.message;
    }

    // 2. Fallback: Intento vía proxy del servidor (puerto 3000)
    try {
        return await fetchUrlPost('http://www.desdetenerife.com:3000/api/admin/agent-proxy', {
            endpoint,
            method,
            payload
        }, adminPass);
    } catch (e) {
        throw new Error(`Directo: ${directError} | Proxy: ${e.message}`);
    }
}
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const response = await fetchUrlPost('http://localhost:3000/api/admin/login', { username, password });
        res.json(response);
    } catch (e) {
...
router.get('/status', async (req, res) => {
    const status = {
        ...
        remote: { status: 'offline', url: 'http://www.desdetenerife.com:3000' },
        os: {
    ...
    // Check Local Server
    try {
        const localHealth = await fetchUrl('http://localhost:3000/api/health');
        status.local.status = 'online';
        // Note: No local database exists in this architecture.
    ...
    try {
        ...
        const remoteHealth = await fetchUrl('http://www.desdetenerife.com:3000/api/health', null, 2500);
...
router.get('/logs', async (req, res) => {
    ...
    try {
        ...
        try { serverLogs = await fs.readFile(SERVER_LOG, 'utf8'); } catch (e) { }
        try { agentLogs = await fs.readFile(AGENT_LOG, 'utf8'); } catch (e) { }
...
router.get('/db-check', async (req, res) => {
    ...
    if (target === 'remote') {
        try {
            const data = await fetchRemoteAgent('/api/admin/db-check?target=local', 'GET');
            return res.json(data);
        } catch (e) {
            return res.json({ status: 'error', message: 'No se puede contactar con el agente de Tenerife' });
        }
    }

    // The system architecture has no local database, only remote
    res.json({ status: 'warning', message: 'No existe base de datos local en esta configuración.' });
});
...
router.post('/execute', async (req, res) => {
    ...
    switch (action) {
        case 'start-server':
            // Comando robusto: Matar si existe, luego arrancar SIEMPRE
            if (isWin) {
                // El comando para matar está en paréntesis para que el resto se ejecute siempre
                ...
                command = 'lsof -ti:3000 | xargs kill -9 2>/dev/null || true; cd server && node app.js > ../server.log 2>&1 &';
            }
            break;
        case 'stop-server':
            if (isWin) {
...
async function fetchUrl(url, adminPass = null, timeout = 5000) {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('timeout', () => {
            req.destroy();
            reject(new Error(`Timeout tras ${timeout}ms (Inaccesible)`));
        });
    });
}
